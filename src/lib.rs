...
use winapi::{BOOL, DWORD, HINSTANCE, HWND, LPCWSTR, LPVOID, TRUE, UINT};
...
fn message_box(title: &str, message: &str) {
    unsafe {
        ...
        let symbol: Func<
            extern "C" fn(hWnd: HWND, lpText: LPCWSTR, lpCaption: LPCWSTR, uType: UINT) -> c_int,
        > = lib.find_func("MessageBoxW").unwrap();
        let mbw = symbol.get();
        mbw(
            0 as HWND,
